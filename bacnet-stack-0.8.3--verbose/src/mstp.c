...
// 2017-03-02 - added by Ted, provides MAX_MPDU . . . seems to solve this problem:
#include <dlmstp.h>
    ...
    Test * pTest)
{
    ...
    ct_test(pTest, mstp_port.SilenceTimer(0) == 0);   // 2017-03-03 - was ct_test(pTest, mstp_port.SilenceTimer() == 0);
    ...
    for (i = 0; i < len; i++) {
        ...
        MSTP_Receive_Frame_FSM(&mstp_port);
        ct_test(pTest, mstp_port.DataAvailable == false);
        ct_test(pTest, mstp_port.SilenceTimer(0) == 0);  // was ct_test(pTest, mstp_port.SilenceTimer() == 0);   - TMH
        ct_test(pTest, mstp_port.EventCount == EventCount);
    }
    ct_test(pTest, mstp_port.ReceivedInvalidFrame == true);
...
//  error: too few arguments to function ‘mstp_port.SilenceTimer’
//     ct_test(pTest, mstp_port.SilenceTimer() == 0);
//                    ^

// 2017-03-02 THU . . .
//        ct_test(pTest, mstp_port.SilenceTimer((void *) mstp_port) == 0);  // was ct_test(pTest, mstp_port.SilenceTimer() == 0);   - TMH
//  error: cannot convert to a pointer type
//         ct_test(pTest, mstp_port.SilenceTimer((void *) mstp_port) == 0);
//         ^

// 2017-03-03 FRI . . .
//        ct_test(pTest, mstp_port.SilenceTimer(void) == 0);
//  error: expected expression before ‘void’
//         ct_test(pTest, mstp_port.SilenceTimer(void) == 0);
//                                               ^

// Hmm interesting, this compiles without error:
        ct_test(pTest, mstp_port.SilenceTimer(0) == 0);
