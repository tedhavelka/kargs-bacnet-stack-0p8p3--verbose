...
// 2017-03-08 WED - Ted amending default ports/linux baud rate, for bench top set up:
...
//    if ( RS485_Baud != B9600 )
    {
        printf("RS485_Get_Baud_Rate():  - WARNING - RS485_Baud found not equal to B9600 as earlier assigned!\n");
        printf("RS485_Get_Baud_Rate():  - BEFORE CHANGE - changing here to B9600,\n");
        RS485_Baud = B9600;
        printf("RS485_Get_Baud_Rate():  - AFTER CHANGE - RS485_Baud unsigned int now holds %u,\n", RS485_Baud);
    }
...
uint32_t RS485_Get_Port_Baud_Rate(volatile struct mstp_port_struct_t * mstp_port)
{
    ...
    printf("RS485_Get_Port_Baud_Rate():  starting,\n");
    printf("RS485_Get_Port_Baud_Rate():  called to return baud rate,\n");
    printf("RS485_Get_Port_Baud_Rate():  looking at mstp_port->UserData . . .\n");

    SHARED_MSTP_DATA *poSharedData = (SHARED_MSTP_DATA *) mstp_port->UserData;
    if (!poSharedData)
    {
        printf("RS485_Get_Port_Baud_Rate():  local data pointer poSharedData is null, returning to caller . . .\n");
        return 0;
    }
